...
public class UrlUtils {
     ...
     * @return
     */
    public static String replaceUrlParamReg(String url, String name, String newValue) {
    ...
    public static String getUrlParam(String url, String paramName) {
    	String[] hostAndParam = url.split("\\?");
    	if (hostAndParam.length != 2) {
    		return null;
    	}
    	String param = hostAndParam[1];
    	String[] params = param.split("&");
    	String result = null;
    	if (ArrayUtils.isNotEmpty(params)) {
    		for (String paramNVStr : params) {
    			if (StringUtils.isNotBlank(paramNVStr)) {
    				String[] nameAndValue = paramNVStr.split("=");
    				if (StringUtils.equals(nameAndValue[0], paramName)) {
